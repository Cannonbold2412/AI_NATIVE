...
function downloadInstaller(pluginId: string, filename: string): void {
  const anchor = document.createElement('a')
  anchor.href = installerDownloadUrl(pluginId)
...
export function BuildInstallerPage() {
  ...
  const installerFilename = currentResult?.filename ?? selectedPlugin?.installer?.filename ?? 'installer.exe'
  ...
  async function handleBuildInstaller() {
    ...
    try {
      const response = await buildInstaller(selectedPlugin.id)
      const result = await readPluginSse<InstallerBuildResult>(response, (message) => {
  ...
  function handleDownloadInstaller() {
    if (!selectedPlugin || !installerReady) return
    downloadInstaller(selectedPlugin.id, installerFilename)
  }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <PageHeader title="Build Installer" description="Turn a built plugin package into a Windows installer and download it after the build finishes." />
      <div className="flex min-h-0 flex-1 gap-4 p-6">
        <div className="flex min-h-0 w-80 flex-col gap-3 rounded-lg border border-white/8 bg-white/[0.03]">
          <div className="border-b border-white/8 px-4 py-3">
            ...
            <p className="mt-0.5 text-xs text-zinc-500">{builtPlugins.length} ready for installer build</p>
          </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        >
                          {plugin.installer ? 'installer' : 'package'}
                        </Badge>
                      </div>
          ...
          {selectedPlugin ? (
            <>
              ...
              <div className="flex flex-wrap gap-2 px-4">
                <Button size="sm" onClick={handleBuildInstaller} disabled={buildingSelected}>
                  {buildingSelected ? (
                    <>
