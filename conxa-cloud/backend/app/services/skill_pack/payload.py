...
# Bare CSS ID selectors (e.g. #_r_1g_) are auto-generated by React/Radix and are
# unstable — passing them to the LLM causes it to copy them verbatim, which then
# fails fill-selector validation. Strip these for fill steps so the LLM uses context.
_CSS_BARE_ID_RE = re.compile(r"^#[\w-]+$")
...
def _sanitize_recording_step_for_llm(step: dict[str, Any]) -> dict[str, Any] | None:
    ...
    # For fill/type actions, derive a stable attribute selector from the DOM target.
    # Priority: name > aria_label > placeholder > existing selectors (xpath removed).
    action_type = _get_step_action_type(c)
    _target = c.get("target") or {}
    target_name = _target.get("name") or ""
    target_aria = _target.get("aria_label") or ""
    target_placeholder = _target.get("placeholder") or ""
    if action_type in {"type", "fill", "input"}:
        if target_name:
            c["selectors"] = {"selector": f'input[name="{target_name}"]'}
        elif target_aria:
            c["selectors"] = {"selector": f'input[aria-label="{target_aria}"]'}
        elif target_placeholder:
            c["selectors"] = {"selector": f'input[placeholder="{target_placeholder}"]'}
        else:
            selectors = c.get("selectors")
            if isinstance(selectors, dict):
                selectors.pop("xpath", None)
                # For fill steps, skip bare CSS IDs like #_r_1g_ — they're
                # auto-generated and will cause the LLM to copy them verbatim,
                # failing validation. text_based selectors are repairable.
                best = next(
                    (
                        v for k in _SELECTOR_PRIORITY
                        if (v := selectors.get(k))
                        and not (k == "css" and _CSS_BARE_ID_RE.match(str(v)))
                    ),
                    None,
                )
                if best:
        ...
        if isinstance(selectors, dict):
            selectors.pop("xpath", None)
            # Strip bare CSS IDs even for non-fill steps — keydown/change steps on
            # the same element will expose #_r_1g_ to the LLM which then uses it
            # when condensing them into a fill step.
            best = next(
                (
                    v for k in _SELECTOR_PRIORITY
                    if (v := selectors.get(k))
                    and not _CSS_BARE_ID_RE.match(str(v))
                ),
                None,
            )
            if best:
